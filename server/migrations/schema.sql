...
-- Add parentId to existing documents table (keeps schema idempotent)
ALTER TABLE documents ADD COLUMN IF NOT EXISTS "parentId" UUID REFERENCES folders(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_folders_user_parent ON folders("userId", "parentId");
CREATE INDEX IF NOT EXISTS idx_documents_parent ON documents("parentId");
